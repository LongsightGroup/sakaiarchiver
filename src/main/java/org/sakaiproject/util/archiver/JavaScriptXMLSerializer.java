 ...
 * @version $Revision: 8371 $
 * @author Ahmed Ashour
 * @author Ronald Brill
 * @author Marc Guillemot
 */
class JavaScriptXMLSerializer {
    ...
    /**
     * Kludge to replace protected function in HTMLUnit
     *
     * @param node
     * @return
     */
    public boolean isEmptyXmlTagExpanded( DomElement node ) {
        final List<String> acceptableTagNames = Arrays.asList(
                new String[]{"a", "dd", "div", "iframe", "ol", "p", "script", "ol",
                		     "span", "style", "table", "textarea", "title", "ul"});

        if (acceptableTagNames.contains(node.getTagName())) {
        	return true;
        }
    	return false;
    }
